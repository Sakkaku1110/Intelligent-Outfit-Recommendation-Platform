...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Rank outfit combinations with a trained compatibility model."
    )
...
def score_candidates(
    ...
    for start in range(0, len(candidates), batch_size):
        ...
        categories = torch.zeros(len(candidate_batch), max_items, dtype=torch.long)
        ...
        with torch.inference_mode():
            logits = model(images.to(device), categories.to(device), mask.to(device))
